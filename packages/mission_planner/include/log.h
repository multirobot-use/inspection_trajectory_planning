...
public:
    Logger(const int drone_id){
        ...
        file_.open(home_path+"/"+string_time+"_drone"+std::to_string(drone_id));
    ...
    void log(std::vector<state> trajectory, const std::string &name =""){
        ...
        for(auto &point : trajectory){
            file_<<point.pos(0) <<", "<<point.pos(1)<<", "<<point.pos(2)<<", "<<
                   ...
                   point.acc(0) <<", "<<point.acc(1)<<", "<<point.acc(2)<<std::endl;
        }
        file_<<std::endl;
    }
    void log(state state_to_log, const std::string &name =""){
        ...
        std::chrono::duration<double> elapsed_seconds = final_time-initial_time;
        file_ << elapsed_seconds.count() <<" " <<name<< ": "<<std::endl;
        file_<<state_to_log.pos(0) <<", "<<state_to_log.pos(1)<<", "<<state_to_log.pos(2)<<", "<<
                   state_to_log.vel(0) <<", "<<state_to_log.vel(1)<<", "<<state_to_log.vel(2)<<", "<<
                   state_to_log.acc(0) <<", "<<state_to_log.acc(1)<<", "<<state_to_log.acc(2)<<std::endl;
        file_<<std::endl;
    }
private:
    std::ofstream file_;
